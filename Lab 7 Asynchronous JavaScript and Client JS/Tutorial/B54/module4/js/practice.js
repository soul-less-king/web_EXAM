...
async function addTransaction(event) {
    ...
    await fetch(API_URL, config); //add transaction to the server
    loadTransactions()
...
async function startEdit(id) {
    ...
    // go to the server and get this data

    const response = await fetch(`${API_URL}/${id}`)
    const transaction = await response.json() //the single transaction
...
async function editTransaction(event) {
    ...
    await fetch(`${API_URL}/${transaction.id}`, config); //add transaction to the server
    loadTransactions()
